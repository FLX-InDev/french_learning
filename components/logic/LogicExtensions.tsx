...
import {
  ...
  type SudokuBoard, type Maze, type DeductionPuzzle,
...
function SudokuBoard() {
  ...
  function check() {
    ...
    setChecked(true); setMsg(ok ? "🎉 完成！" : "还有不对的地方，再试试");
    playSfx(ok ? "correct" : "encourage");
  }
  return <div className="space-y-3 max-w-[320px] mx-auto">
    <div className="text-sm text-gray-500 text-center">{size} 宫数独 · 填 {size === 4 ? "1–4" : "1–6"}</div>
    ...
    <div className="flex gap-2 justify-center">
      <button className="btn-primary" onClick={check} disabled={checked}>检查</button>
      <button className="btn-secondary" onClick={() => { const d = generateSudoku(mulberry32(seedFromString("sudoku_" + size + "_" + Date.now()))); setData(d); setBoard(d.puzzle.map((r) => r.slice())); setChecked(false); setMsg(""); }}>新题</button>
    </div>
...
function MazeGame() {
  ...
  return <div className="space-y-3 max-w-sm mx-auto">
    <div className="text-sm text-gray-500 text-center">移动 🦊 到右下角 ⭐</div>
    ...
    {won && <div className="text-center text-green-600 font-bold">🎉 走出迷宫！</div>}
    <div className="flex gap-2 justify-center">
      ...
      <button className="btn-secondary" onClick={() => { setMaze(generateMaze(mulberry32(Date.now()))); setPos([0, 0]); setWon(false); }}>新迷宫</button>
    </div>
  </div>;
}
...
function DeductionGame() {
  ...
  return <div className="space-y-3 max-w-sm mx-auto">
    <div className="text-sm text-gray-500 text-center">点击线索排除，最后指认嫌疑人</div>
    {data.clues.map((c, i) => <button key={i} onClick={() => setEliminated((s) => new Set(Array.from(s).concat([c.eliminates])))} className={"w-full rounded-xl border-2 p-3 text-left text-sm transition " + (eliminated.has(c.eliminates) ? "bg-green-50 border-green-200" : "bg-white border-gray-100 hover:border-purple-300")}><div className="font-bold text-purple-600">{c.text.zh}</div><div className="text-xs text-gray-400">{c.text.fr}</div></button>)}
    <div className="flex gap-2 justify-center">{data.suspects.map((s) => <button key={s.id} onClick={() => { if (!eliminated.has(s.id)) setGuess(s.id); }} disabled={!!result || eliminated.has(s.id)} aria-pressed={guess === s.id} className={(eliminated.has(s.id) ? "opacity-30" : guess === s.id ? "ring-2 ring-purple-400" : "") + " rounded-xl border-2 p-2 text-center min-w-[64px] " + (result ? (s.id === data.answer ? "border-green-400 bg-green-50" : "border-red-200 bg-red-50") : "border-gray-100 bg-white")}><div className="text-2xl">{s.emoji}</div><div className="text-[10px]">{s.label.zh}</div></button>)}</div>
    {!result && <button className="btn-primary w-full" onClick={check} disabled={!guess}>指认</button>}
    {result !== null && <div className={"text-center font-bold " + (result ? "text-green-600" : "text-orange-500")}>{result ? "🎉 答对了！" : "再想想"} {!result && <button className="btn-secondary mt-2" onClick={() => { setData(generateDeduction(mulberry32(Date.now()))); setEliminated(new Set()); setGuess(null); setResult(null); }}>新题</button>}</div>}
  </div>;
}
...
export function LogicExtensions() {
  ...
  return <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
    <h2 className="text-lg font-bold text-gray-800 mb-3">🧩 更多挑战</h2>
    <div className="flex gap-2 mb-4">
      {TABS.map((k) => { const l = k === "sudoku" ? "🔢 数独" : k === "maze" ? "🌀 迷宫" : "🕵️ 推理"; return <button key={k} onClick={() => setTab(k)} aria-pressed={tab === k} className={"text-xs px-3 py-1.5 rounded-full font-semibold transition min-h-[36px] " + (tab === k ? "bg-purple-600 text-white" : "bg-purple-50 text-purple-600")}>{l}</button>; })}
    </div>
