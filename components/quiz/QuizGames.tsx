...
export function MatchGame({ words: src }: { words: Word[] }) {
  ...
  return <div className="grid grid-cols-2 gap-4 max-w-lg mx-auto">
    ...
    {matched.size === pool.length * 2 && <div className="col-span-2 text-center text-green-600 font-bold">🎉 全部配对成功！</div>}
  </div>;
}
...
export function MemoryGame({ words: src }: { words: Word[] }) {
  ...
  return <div className="grid grid-cols-4 gap-2 max-w-sm mx-auto">
    ...
    {matched.size === src.length && <div className="col-span-4 text-center text-green-600 font-bold">🎉 全部配对成功！</div>}
  </div>;
}

// ── 词块排序（OrderGame，T5C.1）：拖拽/点选排序为正确句子 ──

export function OrderGame({ sentence }: { sentence: { fr: string; zh: string } }) {
  ...
  return <div className="space-y-3 max-w-md mx-auto">
    <div className="text-sm text-gray-500 text-center">排序成正确句子：{sentence.zh}</div>
    ...
    {correct === null && <button className="btn-primary w-full" onClick={check}>检查</button>}
    {correct === false && <div className="text-center text-sm text-orange-500">再试试，拖动词块调整顺序</div>}
    {correct && <div className="text-center text-green-600 font-bold">🎉 排序正确！</div>}
  </div>;
}
...
export function VerticalForm({ a, b, op }: { a: number; b: number; op: "+" | "-" }) {
  ...
  return <div className="space-y-3 max-w-[200px] mx-auto">
    ...
    <div className="border-t-2 border-gray-300 pt-1">
      {carry && <div className="text-xs text-amber-600 text-right">进位 +1</div>}
    ...
    {correct !== null && <div className={"text-center text-sm font-bold " + (correct ? "text-green-600" : "text-orange-500")}>{correct ? "✅ 正确！" : "再算一次"}</div>}
  </div>;
}